...
1. Start a REPL with `clj -M:nrepl`
2. Start the server within the REPL in `src/microservice_boilerplate/server.clj` with `(start-system! (build-system-map))`
3. In your browser's JavaScript console

> You can use protocols field to identity your requests, all data sent in the protocols array will be available in the header `Sec-WebSocket-Protocol`

```javascript
w = new WebSocket("ws://localhost:3001/ws", ["your-token-or-id"])
