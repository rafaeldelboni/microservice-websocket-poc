(ns microservice-boilerplate.components.reitit-pedestal-jetty
...
(defrecord WebServer [service-map config router websocket]
  component/Lifecycle
  (start [this]
    (let [websocket-paths (websocket/build-paths websocket)
          websocket-endpoints (websocket/build-ws-endpoints-fn websocket-paths)
          {:webserver/keys [port allowed-origins]
      ...
      (assoc this :webserver
             (-> (base-service port allowed-origins)
                 ...
                 (merge service-map)
                 (merge {::server/container-options {:context-configurator websocket-endpoints}})
