...
(defn store-raw-request []
  {:name ::store-raw-request
   :enter (fn [ctx]
            (let [body (read-all-bytes (-> ctx :request :body))
                  raw-request (-> (:request ctx)
                                  (select-keys
                                   [:protocol :async-supported? :remote-addr
                                    :params :headers :server-port :content-length
                                    :query-params :content-type :path-info
                                    :character-encoding :uri :server-name
                                    :query-string :path-params :body
                                    :scheme :request-method :context-path])
                                  (assoc :raw-body body))]
              (-> ctx
                  (assoc-in [:request :body] body)
                  (assoc-in [:request :raw] raw-request))))})

(defn read-raw-request []
  {:name ::store-raw-request
   :enter (fn [ctx]
            (let [raw-body (slurp (-> ctx :request :raw :raw-body))]
              (-> ctx
                  (assoc-in [:request :raw :raw-body] raw-body))))})
